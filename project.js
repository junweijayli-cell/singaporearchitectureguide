(function () {
  ...
  if (attribution) {
    attribution.textContent =
      guide.attribution?.photos ||
      "Project photographs and drawings are stored locally for this guide. Map base by OpenStreetMap contributors.";
  }
  ...
  const galleryImages = (project.assets.gallery || []).filter(Boolean);
  const hasPlan = Boolean(project.assets.plan);
    ...
    .map(
      (image, index) => `
        <figure class="detail-figure">
          <img src="${escapeHtml(image)}" alt="${escapeHtml(project.name)} insight image ${index + 1}">
          <figcaption>
            <span>${index === 0 ? "Spatial Insight" : `Insight ${index + 1}`}</span>
            <strong>${index === 0 ? "Closer reading" : "Additional architectural view"}</strong>
            <p>${index === 0 ? "An additional image for reading atmosphere, facade, or interior character." : "A supporting image that helps read section, material, or public-space experience."}</p>
          </figcaption>
        </figure>
      `
    )
  ...
  const defaultPlanLabel =
    project.assets.planType === "footprint" ? "Building footprint diagram" : "Typical floor plan";
  const defaultPlanDescription =
    project.assets.planType === "footprint"
      ? "A building-footprint diagram used where a published floor plan was not publicly accessible."
      : "An architectural floor plan that clarifies how the project's defining spatial features are organized.";
  const planMarkup = hasPlan
    ? `
        <figure class="detail-figure detail-figure--plan">
          <img src="${escapeHtml(project.assets.plan)}" alt="${escapeHtml(project.name)} plan drawing">
          <figcaption>
            <span>Key Plan</span>
            <strong>${escapeHtml(project.assets.planLabel || defaultPlanLabel)}</strong>
            <p>${escapeHtml(project.assets.planDescription || defaultPlanDescription)}</p>
          </figcaption>
        </figure>
      `
    : "";
