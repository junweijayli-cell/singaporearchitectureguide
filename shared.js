(function () {
  ...
  function getMapPosition(location) {
    ...
    const pixelX = worldX - bounds.xBase * 256;
    const pixelY = worldY - bounds.yBase * 256;
  ...
  function buildSourceLinks(project) {
    ...
    if (Array.isArray(sources.gallery) && sources.gallery[0] && String(sources.gallery[0]).startsWith("http")) {
      links.push(`<a href="${escapeHtml(sources.gallery[0])}" target="_blank" rel="noreferrer">Gallery image source</a>`);
    }
